...
print(getwd())
sample_data_path <- system.file("inst/extdata/sample_raw_16S_data", package = "CompHeatmaps")
sample_output_path <- system.file("inst/extdata/filtered_reads", package = "CompHeatmaps")
...
# For the sake of this vignette, we will use a saved output as this step takes a long time
saved_output_path <- system.file("data/outputs/results", package = "CompHeatmaps")
dada_output <- readRDS(saved_output_path)
```
Below is what the output looks like while it is running with verbose set to TRUE. We can also see
progress of the 4 samples being piped through dada2's `dada` function.
```
